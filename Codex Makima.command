...
if [[ ! -r "$bootstrap" ]]; then
  print -u2 -r -- "无法启动 Codex 玛奇玛主题：缺少启动组件 $bootstrap"
...
if [[ ! -r "$entrypoint" ]]; then
  print -u2 -r -- "无法启动 Codex 玛奇玛主题：缺少程序文件 $entrypoint"
  print -u2 -r -- "请重新下载完整的主题目录，不要单独移动此 .command 文件。"
...
"$CODEX_SKIN_NODE" "$entrypoint" start
exit_code=$?
if (( exit_code != 0 )); then
  print -u2 -r -- ""
  print -u2 -r -- "Codex 玛奇玛主题启动失败（退出码 $exit_code）。上方错误信息已保留。"
  print -u2 -r -- "诊断命令：\"$CODEX_SKIN_NODE\" \"$entrypoint\" doctor"
  codex_skin_pause_on_error
fi
exit "$exit_code"
