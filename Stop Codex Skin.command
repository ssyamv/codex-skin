...
if [[ ! -r "$bootstrap" ]]; then
  print -u2 -r -- "无法停止 Codex 玛奇玛主题：缺少启动组件 $bootstrap"
...
if [[ ! -r "$entrypoint" ]]; then
  print -u2 -r -- "无法停止 Codex 玛奇玛主题：缺少程序文件 $entrypoint"
  print -u2 -r -- "请恢复完整的主题目录后重试；主题可能仍在运行。"
...
"$CODEX_SKIN_NODE" "$entrypoint" stop
exit_code=$?
if (( exit_code != 0 )); then
  print -u2 -r -- ""
  print -u2 -r -- "Codex 玛奇玛主题停止失败（退出码 $exit_code）。主题可能仍在运行。"
